...
DEFAULT_METRICS = {'f1'}  # 'bleu-4', 'accuracy',
...
ALL_METRICS = DEFAULT_METRICS | ROUGE_METRICS | DISTINCT_METRICS # | BLEU_METRICS
...
try:
    ...
    print("User doesn't have py-rouge installed, so we can't use it. We'll just turn off rouge computations")
    rouge = None
...
class F1Metric(AverageMetric):
    ...
    @staticmethod
    def compute(guess: str, answer: str) -> 'F1Metric':
        if guess is None or answer is None:
            return AverageMetric(0, 0)
        g_tokens = normalize_answer(guess).split()
        precision, recall, f1 = F1Metric._prec_recall_f1_score(g_tokens, normalize_answer(answer).split())

        return F1Metric(f1, 1)


class ExactMatchMetric(AverageMetric):
    @staticmethod
    def compute(guess: str, answer: str) -> 'ExactMatchMetric':
        if guess is None or answer is None:
            return None
        guess = normalize_answer(guess)
        if guess == normalize_answer(answer):
...
class BleuMetric(AverageMetric):
    @staticmethod
    def compute(guess: str, answer: str, k: int = 4) -> Optional['BleuMetric']:
        """
...
class RougeMetric(AverageMetric):
    ...
    def compute_many(
        ...
        try:
            score = RougeMetric._evaluator.get_scores(normalize_answer(guess), normalize_answer(answer))
        except LookupError:
            print("'ROUGE requires nltk punkt tokenizer. Please run `python -c import nltk; nltk.download(\'punkt\')`)")
            return None, None, None

        scores_rouge1 = score['rouge-1']['r']
        scores_rouge2 = score['rouge-2']['r']
        scores_rougeL = score['rouge-l']['r']
        return (
...
def dict_report(report: Dict[str, Metric]):
    return {k: round(v.value(), 4) if isinstance(v, Metric) else v for k, v in report.items()}
...
class TeacherMetrics(Metrics):
    ...
    def evaluate_response(self, prediction: List[str], labels: List[str]) -> None:
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument("--file", default='data/voken-generate-test.txt', type=str)
    parser.add_argument("--fname", default='fairseq', type=str)
    args = parser.parse_args()

    sys_toks, ref_toks = read_file(args.file)
    if args.fname == 'diffks':
        sys_toks, ref_toks = read_diffks(args.file)
    f1 = []
    metrics = TeacherMetrics('all')
    for guess, answer in tqdm(zip(sys_toks.values(), ref_toks.values())):
        metrics.evaluate_response(guess, answer)

    print(dict_report(metrics.report()))
